...
adata <- ad$read_h5ad("hcc.h5ad")

## Plotting some canonical markers ---------------------------------------------
# dir.create(path = "14_hcc_cell-typing_outs")
# pdf(file = "14_hcc_cell-typing_outs/umap_by_canonical_markers.pdf", width = 12, height = 10)
plot_embedding(
...
# pdf(file = "14_hcc_cell-typing_outs/umap_by_other_variables.pdf", width = 6, height = 5)
plot_embedding(
  source = adata$obs$slide,
  embedding = adata$obsm["X_umap"],
  rasterize = T, labels_discrete = F
) + 
  labs(x = "UM1", y = "UM2", title = "HCC: slide")
plot_embedding(
  source = adata$obs$patient,
  embedding = adata$obsm["X_umap"],
  rasterize = T, labels_discrete = F
) + 
  labs(x = "UM1", y = "UM2", title = "HCC: patient")
plot_embedding(
  source = adata$obs$sample_ID,
  embedding = adata$obsm["X_umap"],
  rasterize = T, labels_discrete = F
) + 
  labs(x = "UM1", y = "UM2", title = "HCC: sample ID") + 
  guides(color = guide_legend(ncol = 1, override.aes = list(size = 3, shape = 15)))
# dev.off()


## Clusters that I think look good ---------------------------------------------
# pdf(file = "14_hcc_cell-typing_outs/umap_by_leiden_clusters.pdf", width = 6, height = 5)
plot_embedding(
  source = adata$uns$iterative_clustering$leiden_res_1.8,
  embedding = adata$obsm["X_umap"],
  rasterize = T, labels_discrete = T
) + 
  labs(x = "UM1", y = "UM2", title = "HCC: leiden clustering (res = 1.8)") + 
  guides(color = guide_legend(ncol = 1, override.aes = list(size = 3, shape = 15)))
# dev.off()


## Autofluorescence score ------------------------------------------------------
adata <- anndataR::read_h5ad("hcc.h5ad", mode = "r+")
...
ggplot() + 
  ...
  theme_classic()
# ggsave(filename = "14_hcc_cell-typing_outs/umap_by_AF_score.pdf", width = 6, height = 5)

# Clusters 8 and 19 are RBCs.


## Finding cluster markers with presto GLMM ------------------------------------
# Pseudo-bulking
library(data.table)
idx <- !(adata$uns$iterative_clustering$leiden_res_1.8 %in% c("8", "19", "1", "4", "5", "6", "7", "13", "14", "17", "21"))
psb <- presto::collapse_counts(counts_mat = (Matrix::t(adata$layers$counts[idx,]) |> as("CsparseMatrix") |> magrittr::set_rownames(value = adata$var_names)),
                               meta_data = data.frame("sample_ID" = adata$obs$sample_ID[idx],
                                                      "cluster" = adata$uns$iterative_clustering$leiden_res_1.8[idx, drop = T]),
                               get_norm = F,
...
saveRDS(object = presto_res, file = "14_hcc_cell-typing_outs/presto_model_psb.RDS")
presto_res <- readRDS(file = "14_hcc_cell-typing_outs/presto_model_psb.RDS")
...
p
# ggsave(filename = "14_hcc_cell-typing_outs/cluster_markers_by_presto_glmm_volcano_plots.pdf", height = 10, width = 12)
...
BPCells::plot_dot(source = Matrix::t(adata$layers$lognorm[idx,]) |> as("CsparseMatrix") |> magrittr::set_rownames(value = adata$var_names),
                  groups = adata$uns$iterative_clustering$leiden_res_1.8[idx, drop = T], 
...
# ggsave(filename = "14_hcc_cell-typing_outs/cluster_markers_by_presto_glmm_bubble_plot.pdf", height = 5, width = 16)

# Looking at some cores: 
d <- adata$obs
d$cluster <- adata$uns$iterative_clustering$leiden_res_1.8

# pdf(file = "14_hcc_cell-typing_outs/clusters_in_space.pdf", height = 10, width = 10)
tinyplot::plt(y_global ~ x_global | cluster, 
              data = d |> dplyr::filter(sample_ID == "HCC2_coreC3"), 
              col = discrete_palette("stallion"),
              pch = 16, cex = 0.5,
              legend = legend(pt.cex = 2), asp = 1)
tinyplot::plt(y_global ~ x_global | cluster, 
              data = d |> dplyr::filter(sample_ID == "HCC1_coreB3"), 
              col = discrete_palette("stallion"),
              pch = 16, cex = 0.5,
              legend = legend(pt.cex = 2), asp = 1)
# dev.off()
...
adata$obs$celltype_level1 <- dplyr::case_when(
  adata$uns$iterative_clustering$leiden_res_1.8 == 1 ~ "Hepatocyte.Malignant",
  adata$uns$iterative_clustering$leiden_res_1.8 == 2 ~ "Cholangiocyte", 
  adata$uns$iterative_clustering$leiden_res_1.8 == 3 ~ "T.and.NK",
  adata$uns$iterative_clustering$leiden_res_1.8 == 4 ~ "Hepatocyte.Malignant",
  adata$uns$iterative_clustering$leiden_res_1.8 == 5 ~ "Hepatocyte.Malignant",
  adata$uns$iterative_clustering$leiden_res_1.8 == 6 ~ "Hepatocyte.Malignant",
  adata$uns$iterative_clustering$leiden_res_1.8 == 7 ~ "Hepatocyte.Malignant",
  adata$uns$iterative_clustering$leiden_res_1.8 == 8 ~ "Erythrocyte",
  adata$uns$iterative_clustering$leiden_res_1.8 == 9 ~ "Neutrophil",
  adata$uns$iterative_clustering$leiden_res_1.8 == 10 ~ "Endothelial",
  adata$uns$iterative_clustering$leiden_res_1.8 == 11 ~ "Immune.Myeloid", 
  adata$uns$iterative_clustering$leiden_res_1.8 == 12 ~ "Plasma", 
  adata$uns$iterative_clustering$leiden_res_1.8 == 13 ~ "Hepatocyte.Malignant", 
  adata$uns$iterative_clustering$leiden_res_1.8 == 14 ~ "Hepatocyte.Malignant", 
  adata$uns$iterative_clustering$leiden_res_1.8 == 15 ~ "Myocyte", 
  adata$uns$iterative_clustering$leiden_res_1.8 == 16 ~ "Fibroblast", 
  adata$uns$iterative_clustering$leiden_res_1.8 == 17 ~ "Hepatocyte.Malignant", 
  adata$uns$iterative_clustering$leiden_res_1.8 == 18 ~ "B.cell", 
  adata$uns$iterative_clustering$leiden_res_1.8 == 19 ~ "Erythrocyte", 
  adata$uns$iterative_clustering$leiden_res_1.8 == 20 ~ "Mast", 
  adata$uns$iterative_clustering$leiden_res_1.8 == 21 ~ "Hepatocyte.Malignant"
)

# pdf(file = "14_hcc_cell-typing_outs/umap_by_celltype_level1.pdf", width = 6, height = 5)
plot_embedding(
  source = adata$obs$celltype_level1,
  embedding = adata$obsm$X_umap,
  rasterize = T, labels_discrete = F
) + 
  labs(x = "UM1", y = "UM2", title = "HCC: cell types (level 1)") + 
  guides(color = guide_legend(ncol = 1, override.aes = list(size = 3, shape = 15)))
# dev.off()

# Saving
anndataR::write_h5ad(object = adata, path = "hcc.h5ad", mode = "w")
...
hcc_stow <- "~/Library/CloudStorage/OneDrive-MassGeneralBrigham/cosmx/franses-hcc-data/hcc-tmas"
...
hcc_samples <- openxlsx::read.xlsx(xlsxFile = "~/Library/CloudStorage/OneDrive-MassGeneralBrigham/cosmx/franses-hcc-data/hcc-tmas/sample_map.xlsx", sheet = "fov-map")
negprobes$corenumber <- plyr::mapvalues(x = paste(negprobes$slide, negprobes$fov, sep = "_"), from = paste(hcc_samples$flowcell, hcc_samples$fov, sep = "_"), to = hcc_samples$corenumber) |> sprintf(fmt = "%02s")
negprobes$sample_ID <- paste0("HCC", gsub(pattern = "hcc_tma", replacement = "", x = negprobes$slide), "_core", negprobes$corenumber)
negprobes <- negprobes[negprobes$sample_ID %in% unique(stringr::str_split(adata$obs_names, pattern = "_", n = 2, simplify = T)[,2]),]
...
negprobes$HCC1_core11 |> plot() # Nice

# Transforming the coordinates to standard scale, as we did with the segmentations.
offsets <- read.csv(file = "13_hcc_qc_pp_outs/HCC_offsets.csv")
...
segs <- sf::read_sf("hcc_segmentation_local_coordinates.geojson")
...
library(ggplot2)
ggplot() + 
  geom_sf(data = segs$HCC1_core11) + 
  geom_sf(data = negprobes$HCC1_core11, mapping = aes(color = target), size = 0.5) + 
...
cidx <- adata$obs_names[!(adata$obs$celltype_level1 %in% c("Erythrocyte", "Hepatocyte.Malignant", "Cholangiocyte"))]
...
# pdf(file = "14_hcc_cell-typing_outs/umap_by_celltype_insitutype.pdf", width = 8, height = 5)
BPCells::plot_embedding(
  source = celltypes$sup_celltype,
  embedding = adata$obsm$X_umap,
  rasterize = T, labels_discrete = F
) + 
  labs(x = "UM1", y = "UM2", title = "HCC: cell types (InSituType)") + 
...
hmsubtype <- marker_heatmap(fct, featsuse = c("CD8A", "CD8B", "CD4", "CD27", idxs[is.element(idxs, fct$gene)]), 
  ...
  ggplot2::labs(fill = "Mean expression in group", title = "HCC: Cell Sub-type Markers")

# Looks good
print(hmsubtype)
ggplot2::ggsave("14_hcc_cell-typing_outs/hcc_T_and_NK_subtyping.pdf", height = 4, width = 12)
...
# 3737             482             841             297             299             507             409             310             614 
...
# Saving
saveRDS(lymphoid_typing, file = "14_hcc_cell-typing_outs/hieratype_custom_lymphoid_pipeline_results.RDS")
anndataR::write_h5ad(object = adata, path = "hcc.h5ad", mode = "w")
...
celltype_cols <- c(
  'Hepatocyte.Malignant' = 'olivedrab',
  'NK' = 'khaki',
...
#   nsim = 1000
# )
# saveRDS(object = presto_res, file = "14_hcc_cell-typing_outs/presto_model_psb_validation.RDS")
presto_res <- readRDS(file = "14_hcc_cell-typing_outs/presto_model_psb_validation.RDS")
...
p
ggsave(filename = "14_hcc_cell-typing_outs/celltype_markers_by_presto_glmm_volcano_plots.pdf", height = 15, width = 12)
...
ggsave(filename = "14_hcc_cell-typing_outs/celltype_markers_by_presto_glmm_bubble_plot.pdf", height = 5, width = 16)
