...
# dir.create("13_hcc_qc_pp_outs")
# write.csv(x = offsets, file = "13_hcc_qc_pp_outs/HCC_offsets.csv", row.names = F)
...
offsets$origin_x <- case_when(
  offsets$slide %in% c("HCC1") & offsets$corenumber %in% c("core05", "core08", "core09", "core10", "core11", "core14") ~ 0, 
  offsets$slide %in% c("HCC1") & offsets$corenumber %in% c("core16", "core17", "core19", "core20", "core21") ~ 4000, 
  offsets$slide %in% c("HCC2") & offsets$corenumber %in% c("core02", "core03", "core04", "core06", "core08", "core09") ~ 8000, 
  offsets$slide %in% c("HCC2") & offsets$corenumber %in% c("core10", "core11", "core12", "core15", "core17", "core18") ~ 12000, 
)
offsets$origin_y <- c(seq(0, 5*4000, 4000), seq(0, 4*4000, 4000), seq(0, 5*4000, 4000), seq(0, 5*4000, 4000))
...
hcc_samples <- openxlsx::read.xlsx(xlsxFile = "~/Library/CloudStorage/OneDrive-MassGeneralBrigham/cosmx/franses-hcc-data/hcc-tmas/sample_map.xlsx", sheet = 2)
hcc_samples$flowcell <- gsub(pattern = "hcc_tma", replacement = "HCC", x = hcc_samples$flowcell)
hcc_samples$corenumber <- paste0("core", sprintf(hcc_samples$corenumber, fmt = "%02s"))
adata$obs$patient <- plyr::mapvalues(x = paste0(adata$obs$slide, adata$obs$corenumber), from = paste0(hcc_samples$flowcell, hcc_samples$corenumber), to = hcc_samples$patient)
adata$obs$patient <- gsub(pattern = "p", replacement = "", x = adata$obs$patient) |> sprintf(fmt = "%03s")
adata$obs$patient <- paste0("lp", adata$obs$patient) # "lp" means "liver patient"
...
adata$obs$core <- plyr::mapvalues(x = paste0(adata$obs$slide, adata$obs$corenumber), from = paste0(hcc_samples$flowcell, hcc_samples$corenumber), to = hcc_samples$core)

ggplot() + 
  geom_point(data = adata$obs, mapping = aes(x = x_global, y = y_global, color = core), shape = ".") +
  scale_color_manual(values = InSituType::colorCellTypes(names = adata$obs$core |> unique())) +
  coord_fixed() + 
  guides(color = guide_legend(override.aes = list(size = 5, shape = 16)))

adata$obs <- dplyr::relocate(adata$obs, corenumber, .after = slide)
adata$obs <- dplyr::relocate(adata$obs, core, .after = y_global)
glimpse(adata$obs)

anndataR::write_h5ad(object = adata, "hcc.h5ad", mode = "w")


### Organizing the segmentation ------------------------------------------------
segfiles <- list.files(path = "02_proseg_outs", pattern = "shapes.parquet", full.names = T, recursive = T)
segfiles <- segfiles[grepl(pattern = "HCC\\d+", x = segfiles)]
...
sf::write_sf(local_seg, "hcc_segmentation_local_coordinates.geojson", append = F)
# sf::write_sf(global_seg, "hcc_segmentation_global_coordinates.geojson", append = F)
...
adata <- anndataR::read_h5ad(path = "hcc.h5ad", mode = "r+")
...
adata$obs |> dplyr::group_by(sample_ID) |> dplyr::summarise(prop_outlier = mean(counts_outlier)) |> print(n = 23)
# # A tibble: 23 × 2
#   sample_ID   prop_outlier
#   <chr>              <dbl>
# 1 HCC1_coreA4       0.101 
# 2 HCC1_coreB2       0.0693
# 3 HCC1_coreB3       0.0645
# 4 HCC1_coreB4       0.0849
# 5 HCC1_coreC2       0.0883
# 6 HCC1_coreC4       0.0424
# 7 HCC1_coreD2       0.0535
# 8 HCC1_coreD3       0.0693
# 9 HCC1_coreE1       0.179  --> eval --> filter
# 10 HCC1_coreE2      0.0534
# 11 HCC1_coreE3      0.106 
# 12 HCC2_coreB1      0.109 
# 13 HCC2_coreB2      0.0627
# 14 HCC2_coreC1      0.0646
# 15 HCC2_coreC2      0.0697
# 16 HCC2_coreC3      0.0487
# 17 HCC2_coreD1      0.179  --> eval --> filter
# 18 HCC2_coreD2      0.0540
# 19 HCC2_coreE2      0.0381
# 20 HCC2_coreE3      0.0764
# 21 HCC2_coreF1      0.0379
# 22 HCC2_coreF2      0.0735
# 23 HCC2_coreF3      0.101 
...
tinyplot::plt(y_standard ~ x_standard | counts_outlier, 
              data = adata$obs |> dplyr::filter(sample_ID == "HCC2_coreD1"),
              pch = 16, cex = 0.5,
              legend = legend(pt.cex = 2), 
              asp = 1
)
tinyplot::plt(y_standard ~ x_standard | counts_outlier, 
              data = adata$obs |> dplyr::filter(sample_ID == "HCC1_coreE1"),
              pch = 16, cex = 0.5,
              legend = legend(pt.cex = 2), 
              asp = 1
)
tinyplot::plt(y_standard ~ x_standard | counts_outlier, 
              data = adata$obs |> dplyr::filter(sample_ID == "HCC1_coreA4"),
              pch = 16, cex = 0.5,
              legend = legend(pt.cex = 2), 
              asp = 1
)
...
tokeep <- !((adata$obs$counts_outlier) | (adata$obs$volume_outlier) | (adata$obs$sample_ID %in% c("HCC1_coreE1", "HCC2_coreD1")))
mean(tokeep) # 0.8967123

# Need to use reticulate here
ad <- reticulate::import("anndata")
adata$write_h5ad(path = "hcc.h5ad", mode = "w")
...
ggplot() + 
  ...
  ggthemes::theme_hc()
# ggsave(filename = "13_hcc_qc_pp_outs/ELBO_plot.pdf", height = 5, width = 5)
# model$save("13_hcc_qc_pp_outs/scvi_model", overwrite = TRUE)
...
pdf(file = "13_hcc_qc_pp_outs/umap_by_clusters.pdf", width = 6, height = 6)
...
### Final save -----------------------------------------------------------------
adata
# AnnData object with n_obs × n_vars = 119382 × 957
