...
zarr_paths = [p for p in Path(".").rglob("PDAC*/*/*.zarr") if p.is_dir()]
zarr_paths = [str(p) for p in zarr_paths]

!mkdir 03_pdac_concat_outs
...
for z in zarr_paths:
  ...
  key = slide+"_"+core
  sd.read_zarr(z).tables["table"].write_h5ad("03_pdac_concat_outs/"+key+".h5ad")

!ls 03_pdac_concat_outs/*.h5ad
adata_paths = glob.glob("03_pdac_concat_outs/*.h5ad")
